...
<!-- 一句话说明本 PR 做什么。涉及数据结构时，按 schema → types → examples → docs 分别列出改动点 -->
...
- [ ] 遵循 spec-first 变更顺序（schema → types → examples → docs），纯实现变更已说明理由
- [ ] `node scripts/validate-schemas.mjs` 通过（或说明本地无法运行的原因）
- [ ] 新增/修改实体在 `examples/` 有对应样例，字段名与 schema 严格一致
...
## 数字自洽性（如涉及 examples 中的计算）

<!-- 如提交了 menu-plan/PO 样例，请给出从菜单→BOM→聚合→取整的推导过程 -->
