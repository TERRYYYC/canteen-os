...
const server=createServer(async(req,res)=>{try{
 ...
 if(url.pathname.startsWith('/__q/worker/')){
  ...
  const output=Buffer.from(await result.arrayBuffer()),contentType=result.headers.get('content-type')??'';
  const responseRecord=contentType.includes('application/json')?JSON.parse(output):{kind:'binary',contentType,bytes:output.length,sha256:createHash('sha256').update(output).digest('hex')};
  Object.assign(row,{status:result.status,response:responseRecord,head:f.repo.head});await persist();
