...
import {render as renderImport} from '../src/pages/admin/import.ts';import {createPlanRenderer} from '../src/pages/admin/plan.ts';import {createTeamMealsApi} from '../src/api/team-meals.ts';import {bindDraftStore} from '../src/admin/store.ts';
...
async function reset(kind='import'){lang='en';hold=false;pending=false;connect();await paint(kind);}
...
try{
 ...
 for(const raw of ['1.5','','6']){await reset();parse('周一午 原汤 8');hold=true;$('.adm-import-submit').click();for(let n=0;!pending&&n<100;n++)await pause();if(!pending)throw new Error('Source not held');input('.adm-import-servings',raw);hold=false;release();await pause();check(`canceled import action follows current ${raw||'clear'}`,drafts.getDraftPlan(id)===null&&$('.adm-import-submit').disabled===(raw==='1.5')&&$('.adm-import-submit').getAttribute('aria-busy')===null,{disabled:$('.adm-import-submit').disabled,label:$('.adm-import-submit').textContent});}
