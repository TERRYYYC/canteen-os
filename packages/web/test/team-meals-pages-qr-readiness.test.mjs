...
async function mount(response){let printed=0;globalThis.window={print:()=>printed++};globalThis.document={body:new QrElement('body'),createElement:t=>new QrElement(t),createTextNode:t=>new QrElement('',t)};globalThis.fetch=async()=>response();const el=new QrElement('main');document.body.append(el);await render(el,{lang:'en',t:k=>k});return{el,printed:()=>printed,button:()=>el.querySelector('.qr-print')};}
