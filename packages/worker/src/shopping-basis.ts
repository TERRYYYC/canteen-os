...
export async function resolveListRevisions(gh: GitHubClient, head: string, list: ShoppingList, checked = new Set<string>()): Promise<void> {
  ...
  for (const revision of revisions) {
    const key = `${head}:${revision}`;
    if (checked.has(key)) continue;
    await resolveRevision(gh, head, revision, 'basis_unavailable');
    checked.add(key);
  }
}
