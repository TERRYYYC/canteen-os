...
({
  commit: bj.commit,
  searchGuard: /search/.test(sw),                 // issue #114 的早退还在不在
  maxEntries: sw.match(/maxEntries:\d+/g),        // 第一个应为 2
...
2. `sw.js` 里那几处关键片段与本地构建产物一致（守卫、`maxEntries`、`build.json` 不在 precache）。
3. precache 里的入口脚本 == 线上 `index.html` 实际引用的那个文件名。
4. `caches` 里的三方自洽：`publication-manifest` 中 `build.json` 的 `commit`
   == precache 里投影 `week-41.json` 的 `sourceRevision` == 上面第 1 条。
   三者一致，装过应用的客户端断网冷启动才不会撞 `revision_mismatch`（见 issue #110 / #114）。
