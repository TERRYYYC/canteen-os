...
## Original nonauthor design calibration

The original plan_review read fixed994a and related C/D code; existing C open/edit is sufficient, with no C interface change. Its design assessment is not a production approval. The following boundaries are now part of the implementation contract:

1. Return distinct absent/applied/rejected dispositions. Old auth, retired form, permanently closed session and refused edit must never be treated as absence. Bind capability to API, mode, auth and plan. Restore synchronously without the catalog-awaiting full load(); after edit's synchronous subscribers run, recheck Import live/input generation before setting imported or navigating.
2. Preserve invalid raw text for unrelated rows and matched rows whose imported count is omitted. An explicitly imported number or explicitly cleared count intentionally supersedes that matched row's old invalid raw text, so clear that exact row's raw override. Preserve same-key duplicates, first-match behavior and original index0; the temporary mapping follows original positions. Update the view's reload generation before synchronous repaint; restore the prior map if edit refuses.
3. A successful capability application must not subsequently be overwritten by an older whole-plan store entry when returning to Plan. Keep the genuinely absent-document GET/store/first-consumption path and its existing protection/undo behavior. Refusal/read failure/language cancellation retains raw input and settles only the original read ticket.
4. Saving/unknown/conflict import uses edit, not replace. Preserve original request body, condition and operation; old ACK cannot clean imported later edits, unknown recovery verifies only the old request, and conflict remains unresolved. Check actual static dependencies and the432-byte Plan margin early.
