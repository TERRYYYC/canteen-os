...
- 合了什么：**零代码增量。** 06:20Z 合了 PR #93（`docs/ops/sandbox-headless-chromium.md`，纯文档），推翻 09-10 记的「白名单挡 Playwright CDN」归因——CDN 可达，只缺 `libXdamage.so.1` 一个系统库，20 行 stub 顶掉 4 个符号即可。07:0xZ 的调度轮**在一条全新沙箱上独立复跑了这份配方做复核**：干净克隆 → `pnpm -C packages/core build` → `build-data.mjs` → `pnpm -C packages/web build` → 本地 8787 起服务 → 线上五屏（`#/prep` `#/purchase` `#/menu` `#/admin` `#/qr`）+ reference-v3 原型四屏（`#home` `#menu` `#purchase` `#plan` `#knowledge`）全部截出，**pageerror 0**。配方可复现，视觉验收从此可以写进派工单的硬验收。
