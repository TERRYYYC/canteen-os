...
- 卡在哪：**沙箱好了**（4.1 GB，用会话私有目录不再撑爆）；**子 agent 被网络错误打断了三次**，其中 #20a 两次——第二次是在 blob SHA 核对完最后一个文件、开 PR 之前，所以派工单改成「早推、早开 draft PR、每次推完核 SHA」，之后八个 agent 零丢失。三个搬运坑记下来：① `push_files` 会把源码里的 U+0000、U+0300–U+036F 这类转义序列解成真字符（#22/#23 各踩一次，改成 `String.fromCharCode` / `\p{M}` 等价写法）——**09-11 补注**：这句原文是把那两个转义按字面写进来的，而 09-11 经 GitHub API 重写整份文件时它们真的被解成了控制字符（文件一度被 git 判为 binary），所以改成 U+ 描述写法，含义不变；这条坑对写日志本身同样成立；② `/tmp` 多 agent 共享，冒烟脚本互相覆盖（改成 `/tmp/smoke-<issue>/`）；③ 加依赖会改 lockfile，仍然推不上去——所以 **#22 本轮没加 SheetJS，xlsx 上传延后**，只做粘贴 + 手写 CSV。真机 / 真浏览器一次都没跑过（沙箱无浏览器），版式全靠 jsdom / happy-dom 冒烟（合计 23+85+14+70+42+61 条）。
...
- 合了什么：**零代码增量。** 06:20Z 合了 PR #93（`docs/ops/sandbox-headless-chromium.md`，纯文档），推翻 09-10 记的「白名单挡 Playwright CDN」归因——CDN 可达，只缺 `libXdamage.so.1` 一个系统库，20 行 stub 顶掉 4 个符号即可。07:0xZ 的调度轮**在一条全新沙箱上独立复跑了这份配方做复核**：干净克隆 → `pnpm -C packages/core build` → `build-data.mjs` → `pnpm -C packages/web build` → 本地 8787 起服务 → 线上五屏（`#/prep` `#/purchase` `#/menu` `#/admin` `#/qr`）+ reference-v3 原型五屏（`#home` `#menu` `#purchase` `#plan` `#knowledge`）全部截出，**pageerror 0**。配方可复现，视觉验收从此可以写进派工单的硬验收。
- 卡在哪：**Owner 仍未回答 09-10 那个「不喜欢什么」。** 零 issue 开着，#91 / #92 两条 draft 自 09-10 未动，main 自 `bfc47f7` 无新提交。但并排实拍改写了一条结论：**「线上全面走样、不如设计稿」不成立。** 采购单一屏线上反而更接近能用——供应商是「宏达粮油调味批发 / 绿源农产品配送」带下单提前期、起订量凑整（「起订 20 包 · 实需 292 g · 余量入库」）、上次价、复制微信文本；设计稿那一屏是 Fresh Market / Pantry & Co. 两个英文占位，没有起订逻辑。真正的差距在**产品框架**：设计稿是照片优先、面向食客的应用（首页 hero / 菜单 / 知识库 / 计划 + 底部四项导航 + 默认中文 + 三语切换），线上是默认 EN、图位全为 "no photo yet" 的工单工具。五屏里只有菜单 / 采购 / 计划在同一概念上，设计稿没有备料单和二维码，线上没有首页和知识库。
- 明天派什么：**仍不派实现类任务**，但把二选一扩成**三选一**交给 Owner，回一个字母即可开工：**A** 照设计稿重做前台（保留业务逻辑，只换皮 + 补首页/知识库，硬验收 = PR 必须附本屏截图 + 与 reference-v3 的差异清单）；**B** 方向本身不对，v0.3 就地封存，下一轮只重谈需求不写代码；**C**（本轮实拍后新增）保留线上的厨房作业内核（起订量 / 余量入库 / 上次价 / 微信文本），套设计稿的外壳，再补首页与知识库两屏。并排对照页已在本轮生成（10 张 390×844 @2x 实拍），交给 Owner 直接看图选。
