...
const web='/private/tmp/app-bundle-c1-review-e79fe3b-etbw3e/packages/web',require=createRequire(join(web,'package.json'));
...
test('independent: shared import promise across route ABA and language change starts only latest renderer',async()=>{
 const g=boot('admin'),cache=new Map(),calls=[];g.load=route=>{calls.push(route);if(!cache.has(route))cache.set(route,deferred());return cache.get(route).promise;};
 g.requests[0].resolve(publication);await flush();g.route('purchase','other');g.route('admin','new-rest');g.changeLanguage();
 cache.get('admin').resolve({render:g.render});await flush();
 assert.equal(g.renders.length,1);assert.equal(g.renders[0].ctx.route,'admin');assert.equal(g.renders[0].ctx.rest,'new-rest');assert.equal(g.renders[0].ctx.lang,'zh');
 cache.get('purchase').resolve({render:g.render});await flush();assert.equal(g.renders.length,1);assert.deepEqual(calls,['admin','purchase','admin','admin']);
});
test('independent: import rejection after auth changes starts a fresh loader without painting stale failure',async()=>{
 const g=await ready('admin');g.runtime.clearToken();g.loads[0].reject(Error('old authenticated load failed'));await flush();
 assert.equal(g.loads.length,2);assert.equal(g.renders.length,0);assert.equal(g.outlets.at(-1).children.length,1);assert.equal(g.outlets.at(-1).children[0].textContent,'data.loading');
 g.finish(1);await flush();assert.equal(g.renders.length,1);assert.equal(g.runtime.inspectReloadSafety().reason,'clear');
});
test('independent: latest publication failure replaces a pending editor load with exact null/error context',async()=>{
 const g=await ready('purchase'),refresh=g.pwa.refreshPublication();g.requests[1].reject(Error('publication unavailable'));await refresh;
 assert.equal(g.loads.length,2);g.finish(0);await flush();assert.equal(g.renders.length,0);g.finish(1);await flush();
 assert.equal(g.renders.length,1);assert.equal(g.renders[0].ctx.publication,null);assert.equal(g.renders[0].ctx.planId,null);assert.ok(g.renders[0].ctx.publicationError);
});
test('independent: offscreen started renderer rejection neither paints pending route nor clears unresolved coverage',async()=>{
 const g=await ready('admin'),gate=deferred();g.ack=false;
 g.loads[0].resolve({render(el,ctx){g.renders.push({el,ctx});return gate.promise;}});await flush();
 assert.equal(g.runtime.inspectReloadSafety().reason,'untracked');g.route('menu','');const waiting=g.outlets.at(-1);
 gate.reject(Error('old started renderer failed'));await flush();assert.equal(waiting.children.length,1);assert.equal(waiting.children[0].textContent,'data.loading');
 g.ack=true;g.finish(1);await flush();assert.equal(g.renders.length,2);assert.equal(g.runtime.inspectReloadSafety().reason,'untracked');
});
test('independent: a disconnected outlet never starts renderer or registers page coverage',async()=>{
 const g=await ready('admin');g.outlets.at(-1).isConnected=false;g.finish(0);await flush();assert.equal(g.renders.length,0);assert.equal(g.runtime.inspectReloadSafety().reason,'clear');
});
