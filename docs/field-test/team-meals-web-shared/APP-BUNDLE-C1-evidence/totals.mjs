...
const screen=n=>chunks.find(x=>x.facade?.endsWith('/pages/admin/'+n+'.ts'))?.file;
...
for(const route of ['prep','menu','purchase','qr','locked-admin','home','plan','import','ingredient-new','dish-new','publish']){
 ...
 paths[route]={files,gzipBytes,noSwGzipBytes:gzipBytes-map.get(workbox).finalGzipBytes,within60000:gzipBytes<=60000};
}
...
if(dist){const sw=await readFile(dist+'/sw.js','utf8');total.precachedJs=chunks.filter(x=>sw.includes(x.file)).map(x=>x.file);const workbox=sw.match(/"(\.\/workbox-[^"]+)"/);total.swAndRuntimeGzip=gzipSync(sw).length+(workbox?gzipSync(await readFile(dist+'/'+workbox[1]+'.js')).length:0);}
