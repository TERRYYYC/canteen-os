...
Evidence custody: the generic `*.log` ignore rule was explicitly overridden for the15 original raw logs. Their original terminal whitespace is retained byte-for-byte; the archive-only whitespace check reports those raw-log lines. Source and narrative documents pass the scoped diff check. Every archived evidence file is bound by SHA256SUMS.json; no red output was normalized or relabelled.
