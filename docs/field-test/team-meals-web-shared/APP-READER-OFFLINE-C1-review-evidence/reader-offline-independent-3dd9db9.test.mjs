...
import {publishedFixture} from '/private/tmp/reader-offline-3dd9db9-subreview-svumCQ/packages/web/test/published-fixture.mjs';

const root='/private/tmp/reader-offline-3dd9db9-subreview-svumCQ/packages/web';
const require=createRequire(join(root,'package.json')),esbuild=await import(pathToFileURL(createRequire(require.resolve('vite/package.json')).resolve('esbuild')).href);
...
test('independent: fetch AbortError permits exactly one fully validated ordinary attempt',async()=>{
 const {api,state,calls}=setup();const p=await api.loadPublication({fresh:true});
 state.override=u=>{if(u.pathname.includes('/team-meals/')&&u.search)throw new DOMException('request aborted','AbortError');};
 const view=await project(api,p);assert.equal(view.sourceRevision,A.revision);assert.deepEqual(projections(calls).map(c=>!!c.url.search),[true,false]);
 assert.equal(Object.isFrozen(view),true);assert.equal(Object.isFrozen(view.projection),true);
 await assert.rejects(api.loadPublishedAsset(structuredClone(view),'/ingredients/tomato/image'),{code:'publication_changed'});
 api.clearCache();await assert.rejects(api.loadPublishedAsset(view,'/ingredients/tomato/image'),{code:'publication_changed'});
});
test('independent: errored JSON bodies with HTTP status do not broaden transport-only retry',async()=>{
 for(const reason of [new TypeError('body network failed'),new DOMException('body aborted','AbortError')]){
  const {api,state,calls}=setup();const p=await api.loadPublication({fresh:true});
  state.override=u=>u.pathname.includes('/team-meals/')?new Response(new ReadableStream({start(c){c.error(reason)}}),{headers:{'Content-Type':'application/json'}}):undefined;
  await assert.rejects(project(api,p),e=>{assert.equal(e.code,'invalid_data');assert.equal(e.status,200);assert.ok(new URL(e.url).search);assert.equal(e.sourceRevision,A.revision);return true});
  assert.equal(projections(calls).length,1);
 }
});
test('independent: failure of the ordinary candidate stops at two attempts and does not poison retry',async()=>{
 const {api,state,calls}=setup();const p=await api.loadPublication({fresh:true});
 state.override=u=>{if(u.pathname.includes('/team-meals/'))throw new TypeError('both unavailable')};
 await assert.rejects(project(api,p),e=>{assert.equal(e.code,'unavailable');assert.equal(e.status,null);assert.equal(new URL(e.url).search,'');assert.equal(e.sourceRevision,A.revision);return true});
 assert.equal(projections(calls).length,2);state.override=null;state.online=false;
 assert.equal((await project(api,p)).sourceRevision,A.revision);assert.equal(projections(calls).length,4);
});
test('independent: ordinary refusal or invalid payload cannot trigger a third request',async()=>{
 for(const [reply,code,status] of [[()=>json({},403),'unavailable',403],[()=>new Response('<html>wrong</html>',{headers:{'Content-Type':'text/html'}}),'invalid_data',200],[()=>new Response('{',{headers:{'Content-Type':'application/json'}}),'invalid_data',200]]){
  const {api,state,calls}=setup();const p=await api.loadPublication({fresh:true});state.online=false;
  state.override=u=>u.pathname.includes('/team-meals/')&&!u.search?reply():undefined;
  await assert.rejects(project(api,p),e=>{assert.equal(e.code,code);assert.equal(e.status,status);assert.equal(new URL(e.url).search,'');return true});
  assert.equal(projections(calls).length,2);
 }
});
test('independent: clear triggered by deadline abort prevents obsolete recovery',async()=>{
 const {api,state,calls}=setup({timeoutMs:10});const p=await api.loadPublication({fresh:true});
 state.override=(u,init)=>u.pathname.includes('/team-meals/')?new Promise((_resolve,reject)=>{init.signal.addEventListener('abort',()=>{api.clearCache();reject(new DOMException('aborted','AbortError'));},{once:true})}):undefined;
 await assert.rejects(project(api,p),{code:'publication_changed'});assert.equal(projections(calls).length,1);
 state.override=null;const next=await api.loadPublication();assert.equal((await project(api,next)).sourceRevision,A.revision);
});
test('independent: timed-out tagged body cannot overwrite successful recovery when it later completes',async()=>{
 const {api,state,calls}=setup({timeoutMs:10});const p=await api.loadPublication({fresh:true});let stream;
 state.override=u=>u.pathname.includes('/team-meals/')&&u.search?new Response(new ReadableStream({start(c){stream=c}}),{headers:{'Content-Type':'application/json'}}):undefined;
 const recovered=await project(api,p);assert.equal(recovered.sourceRevision,A.revision);
 stream.enqueue(new TextEncoder().encode(JSON.stringify(B.projection)));stream.close();await new Promise(setImmediate);
 assert.equal(await project(api,p),recovered);assert.equal(projections(calls).length,2);
});
