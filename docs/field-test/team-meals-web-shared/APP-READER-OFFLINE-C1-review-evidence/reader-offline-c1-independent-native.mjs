...
const web='/private/tmp/reader-offline-c1-review-3dd9db9-rmoz_4r6/packages/web',head='3dd9db9d07ec8fdc67990ecbb77a0578f2dc2338';
const directory='/private/tmp/reader-offline-c1-independent-native',port=0;
...
const f=document.querySelector('#appframe'),out=document.querySelector('#result'),controllerEvents=[];let lastObserved='';setInterval(()=>{try{const w=f.contentWindow;if(!w.location.pathname.startsWith('/canteen/'))return;const key=w.performance.timeOrigin+':'+!!w.navigator.serviceWorker.controller;if(key!==lastObserved){lastObserved=key;controllerEvents.push({boot:w.performance.timeOrigin,controller:!!w.navigator.serviceWorker.controller,at:Date.now()});}}catch{}},5);
...
bind('menu',()=>{f.contentWindow.location.hash='#/menu';});bind('prep',()=>{f.contentWindow.location.hash='#/prep';});bind('admin',()=>{f.contentWindow.location.hash='#/admin';f.contentWindow.location.reload();});bind('reload',()=>f.contentWindow.location.reload());
...
createServer(async(req,res)=>{try {
 ...
 }catch(e){res.writeHead(404);res.end();}}).listen(port,'127.0.0.1',function(){console.log(JSON.stringify({url:'http://127.0.0.1:'+this.address().port+'/inspect.html',head,snapshots}));});
