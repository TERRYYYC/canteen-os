...
只有原操作闭包的明确结果能删除其编号：形状合法的 WriteResult，或明确 HTTP 拒绝。4xx 中 timeout/499、bad_response 与 session_changed 不作为未生效证明；网络/5xx 仍未知。结束一个不能删除其它旧操作；最后一个确定结束时才注销旧 provider，不恢复旧正文，也不写新 B。原 real transport 为保护身份把旧响应转成 session_changed 时，C1 不能反向猜测被丢弃的响应是否成功。
