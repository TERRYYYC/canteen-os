...
## Daily-use repair — active, 2026-09-12 18:26 UTC

This supersedes the earlier slice-completion statement for whole-site usability. Dispatch's continuation (source task `01a08d6d-be28-7142-ad8e-3f964658d3f4`, dispatch `9db20a7`) authorizes the confirmed issues in `/Users/terry/Documents/Codex/2026-09-12/canteen-human-usage-audit/outputs/CanteenOS-日常使用体验审查.md`. The original independent audit and screenshots remain unchanged. No new implementation task or agent tree; the original nonauthor reviewer will be reused. The accepted green visual direction and existing data/session contracts remain.

**What / Why:** remove confirmed daily-use blockers, first editor Save obstruction and unreadable preparation content; then connect shopping retrieval/return and reconcile plan/publication states. **Tradeoff:** page presentation/actions/CSS and related tests only, including the newly authorized home, publish, editors, prep and QR. API/auth/shared session/router semantics/core/Worker remain read-only unless a specific minimum change is approved. **Open questions:** a real human-checked representative recipe is not yet available; current photo/name/recipe combinations are incomplete test data. **Next action:** native RED → focused regression protection → clickable Save/Prep checkpoint → remaining confirmed flows → original reviewer and independent user-journey audit. No merge, remote operation or production acceptance is implied.

| Confirmed problem | Diagnosis / planned verification | Status |
|---|---|---|
| Dish/ingredient Save covered | Existing editor bars use sticky bottom 0 while the new fixed navigation uses the same viewport edge; outlet padding alone cannot reserve sticky geometry. Check actual button-center hit targets and keyboard reach at 1280×720 and mobile, before and after a shared navigation clearance fix. | First fix verified natively: desktop and mobile button centers hit Save, not navigation; keyboard reaches Save; empty submit shows validation |
| Prep/Menu/material details expose internal fields | Published issues use JSON.stringify directly; frozen recipe facts render raw enums and all three translations in ordinary content. Replace with current-language names, dates, impact and relevant actions; put version/debug/translation/license information in clearly labelled details. Preserve warnings, unrecorded/to-taste/recorded quantities and revision-bound links. | First current-language presentation fix and regression tests passed; native warning/detail follow-up in progress |
| Timing filter silently removes materials | Filter excludes components whose timing is unrecorded, without an empty explanation or reset. Keep a visible unknown-timing explanation and View all recovery. | Explanation/reset implemented; native final reset check in progress |
| Shopping requires IDs and loses return context | Current API/Worker catalog has no plan or saved-shopping-list index; ordinary entry cannot honestly list all saved server records. Plan handoff and page-local return are within page scope; local history must say “此设备最近打开”. A minimum read-only index proposal is pending below. | Not started |
| Home/Plan/Publish disagree | Home's week-N lookup differs from the validated shared plan context. Reuse a generic actual selected plan and distinguish no unpublished changes / no release record / unknown online state. Never hardcode team-week. | Not started |
| Misleading entries / QR / incomplete sample | Accurately name add-only material/video alternatives; explain unavailable QR and disable printing. Sample disclaimer must explicitly identify incomplete test recipes and mismatched illustration names, never claim human verification. | Not started |

Baseline was clean at `7f824a20ad5f042a8b8cace022274f3ca743ed14` (product `11c8a98`). The expired 4275 preview was confirmed stopped; its original evidence, server and lease record were preserved under `/private/tmp/canteen-hifi-4275/usage-audit-20260912/baseline/` before restarting. Restored running/launchd at 18:21 UTC, lease PID 87995, listener 88023, expires **2026-09-13 02:21:25 UTC**. HTTP returned 200 and all 60 restored runtime TS/CSS hashes match product `11c8a98`. The saved local shopping sample is retained; the previous runtime had no POSTs. This is still the unmodified baseline for reproduction, not the repair preview.

**First slice evidence (18:39 UTC):** native baseline Dish Save centers hit navigation (stored `baseline/save-hit-red.json`); after repair both centers are clear at 1280×720 and 393×852, keyboard reaches Save, and invalid empty submission reaches the existing field error. Ingredient Save is clear at 393×852. Five new readability/empty-recovery tests were first red for their reported reasons, then green. Current related suite **85/85**, TypeScript and diff check passed; no lower-layer matrix rerun. Evidence stays in `/private/tmp/canteen-hifi-4275/usage-audit-20260912/`. No native valid-save or final independent journey acceptance is claimed yet.

### Minimum interface gap — authorized 2026-09-12

Current `getCatalog` / `GET /catalog` cannot enumerate menu plans or saved shopping lists. Cross-device discovery needs an authenticated, read-only index of plan identities/names/date ranges and shopping-list identities/source scopes/update information; reads must use a coherent revision and existing authorization. No write semantics, new database or claim that device history represents server completeness is needed. Dispatch has now authorized D to add only an existing-auth/read-bucket-protected saved-shopping discovery endpoint, its TeamMealsApi method/types and targeted tests. B/C have no competing writer. The existing public manifest will supply plan choices. The exact bounded response/paging contract will be recorded here before implementation; schema/write/If-Match/source semantics/auth remain unchanged. Q owns its original E2E server/fixture; those repository files stay untouched.
