...
add('D1 Prepare real inline load',async()=>{
 ...
 holdTranslation=true;textButton('Change');
...
add('D1 Continue ordinary editing and no-image save',async()=>{
 ...
 const saltOption=[...document.querySelectorAll('#app [role="option"]')].find(x=>x.textContent.includes('Salt'));assert(saltOption,'existing Salt choice');saltOption.click();
 edit('#adm-dish-name-zh','模块失败后仍可保存菜品');const before=ledger.filter(x=>x.method==='POST'&&x.path==='/dish/soup').length;
 click('.adm-dish-save-draft');await wait(()=>ledger.filter(x=>x.method==='POST'&&x.path==='/dish/soup').length===before+1,'real no-image C1 write');await wait(()=>state().reason==='clear','no-image C1 save settles');
