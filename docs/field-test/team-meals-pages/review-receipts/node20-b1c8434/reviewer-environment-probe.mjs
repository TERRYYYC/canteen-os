...
function sameGlobals(expected){for(const [key,value]of expected)assert.deepEqual(descriptor(key),value,key);const now=Object.getOwnPropertyDescriptors(Event.prototype);assert.deepEqual(Reflect.ownKeys(now),Reflect.ownKeys(eventPrototype));for(const key of Reflect.ownKeys(now)){for(const attr of ['value','get','set','writable','enumerable','configurable'])assert.equal(now[key][attr],eventPrototype[key][attr],String(key)+'.'+attr);}}
try{
