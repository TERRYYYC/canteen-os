...
for(const name of ['rich','multi-dish','multi-row','normal','image-a','image-b','quantity-warning','external-image','empty-plan','no-plans']){const fixture=name==='rich'?richFixture():publishedFixture(name);await mkdir(join(root,'fixtures',name),{recursive:true});fixture.publish(join(root,'fixtures',name,'data'));meta[name]={revision:fixture.revision,builtAt:fixture.manifest.builtAt,source:'publishedFixture → approved runBuild; isolated local Git fixture'};fixture.cleanup();}
const html=await readFile(new URL('./review-public-browser.html',import.meta.url),'utf8'),script=html.match(/<script type="module">([\s\S]*?)<\/script>/)[1];
...
const sourceHashes={};for(const file of ['../src/pages/menu.ts','../src/pages/prep.ts','../src/pages/published-meals.ts','./review-public-browser.html','./team-meals-pages-published-fixture.mjs'])sourceHashes[file]=createHash('sha256').update(await readFile(new URL(file,import.meta.url))).digest('hex');await writeFile(join(root,'source-hashes.json'),JSON.stringify(sourceHashes));
...
const port=Number(process.env.D_PUBLIC_PORT??4251);if([3003,3004,6399].includes(port))throw new Error('Reserved port');
