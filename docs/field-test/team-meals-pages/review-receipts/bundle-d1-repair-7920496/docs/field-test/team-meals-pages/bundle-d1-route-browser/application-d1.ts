...
// Author route-fix instrumentation, reusing original reviewer reproduction controls. Actual main.ts and actual page singleton renderers remain unchanged.
...
globalThis.fetch=async(url,init={})=>{const u=new URL(String(url),location.href),method=init.method||'GET';
 ...
 if(method==='POST'){
  ...
  if(u.pathname.startsWith('/image/'))return json({ok:true,src:'data/review-image.png',license:'own'});
...
const panel=document.createElement('aside');panel.id='review-panel';panel.innerHTML='<strong>AUTHOR ROUTE FIX TEST · actual application/main/PWA · simulated API only</strong><div id="review-controls"></div><pre id="review-output"></pre>';
...
add('D1 Select built-in local image fixture',async()=>{
 const response=await nativeFetch(import.meta.env.VITE_D1_PHOTO_URL);assert(response.ok,'formal local PNG fixture');
 const blob=await response.blob(),file=new File([blob],'d1-local-fixture.png',{type:'image/png'}),transfer=new DataTransfer();transfer.items.add(file);
 const input=$('#adm-ing-photo-pick');assert(input,'actual image input');input.files=transfer.files;input.dispatchEvent(new Event('change',{bubbles:true}));
 check('synthetic local file selection starts real page image-module protection',state().reason==='saving');
});
...
let reviewerBeforeWrite=0;
add('Reviewer prepare deferred Ingredient save',async()=>{
 await route('admin/ingredient/salt','#adm-ing-name-zh');await wait(()=>state().reason==='clear','Ingredient reads settled');
 edit('#adm-ing-name-zh','A saved only if still on screen');reviewerBeforeWrite=ledger.filter(x=>x.method==='POST').length;
 click('.adm-ing-save');await pause();check('legacy loading owns read protection before any Ingredient write',state().reason==='saving'&&ledger.filter(x=>x.method==='POST').length===reviewerBeforeWrite);
});
add('Reviewer prepare deferred Dish photo save',async()=>{
 await route('admin/dish/soup','#adm-dish-name-zh');await wait(()=>state().reason==='clear','Dish reads settled');edit('#adm-dish-name-zh','A photo save');
 const r=await nativeFetch(import.meta.env.VITE_D1_PHOTO_URL),blob=await r.blob(),transfer=new DataTransfer();transfer.items.add(new File([blob],'review.png',{type:'image/png'}));
 const picker=[...document.querySelectorAll('#app input')].find(x=>x.type==='file');picker.files=transfer.files;picker.dispatchEvent(new Event('change',{bubbles:true}));
 await wait(()=>state().reason==='dirty'&&$('.adm-dish-preview img'),'real decoded pending Dish photo',15000);
 reviewerBeforeWrite=ledger.filter(x=>x.method==='POST').length;click('.adm-dish-save-draft');await pause();
 check('legacy loading owns read protection before any Dish upload',state().reason==='saving'&&ledger.filter(x=>x.method==='POST').length===reviewerBeforeWrite);
});
add('Reviewer leave while code is pending',async()=>{
 await route('admin/plan/reviewer-away','[data-focus="servings-0"]');edit('[data-focus="servings-0"]','13');
 check('different editor is current while old module read remains protected',state().reason==='saving'&&ledger.filter(x=>x.method==='POST').length===reviewerBeforeWrite);
});
add('Reviewer verify no late transport after departure',async()=>{
 await wait(()=>state().reason!=='saving','old module task settles',15000);await pause(100);
 check('deferred module completion must not begin a write after the original page left',ledger.filter(x=>x.method==='POST').length===reviewerBeforeWrite,{before:reviewerBeforeWrite,posts:ledger.filter(x=>x.method==='POST')});
 check('current Plan raw stays retained',$('[data-focus="servings-0"]').value==='13');
});

let authorInlinePosts=0;
add('Author prepare inline save across language',async()=>{
 await wait(()=>$('.adm-dish-form'),'initial Dish');await wait(()=>state().reason==='clear','initial reads settled');
 if($('.adm-dish-card-toggle').getAttribute('aria-expanded')!=='true')click('.adm-dish-card-toggle');textButton('Change');
 edit('.adm-dish-search input','Temporary seed');click('.adm-dish-result-new');
 // The native import is asynchronous: clear this actual editable input synchronously,
 // so the arriving real form has no name to automatically translate.
 edit('.adm-dish-inline-loading input','');
 await wait(()=>$('.adm-dish-inline'),'native inline form arrived');
 check('inline form became ready without legacy execution or automatic translation',!performance.getEntriesByType('resource').some(r=>/\/client-[^/]+\.js/.test(r.name))&&!ledger.some(r=>r.method==='POST'));
 edit('.adm-dish-inline input[lang="zh"]','语言中的原材料');edit('.adm-dish-inline input[lang="en"]','Language owner');
 authorInlinePosts=ledger.filter(x=>x.method==='POST').length;click('.adm-dish-inline-actions button:last-child');await pause();
 check('inline save waits on real legacy module with original input and no write',state().reason==='saving'&&ledger.filter(x=>x.method==='POST').length===authorInlinePosts);
 await language('uk','.adm-dish-inline');
 check('same inline raw and save operation survive language redraw',$('.adm-dish-inline input[lang="zh"]').value==='语言中的原材料'&&state().reason==='saving');
});
add('Author verify inline language save after real module',async()=>{
 await wait(()=>state().reason!=='saving','inline save after module response',15000);
 const posts=ledger.filter(x=>x.method==='POST');
 check('one original inline save proceeds after language-only redraw',posts.length===authorInlinePosts+1&&posts.at(-1).path==='/ingredient/language-owner',{posts});
 check('current Dish row uses acknowledged ingredient without resurrecting old form',!$('.adm-dish-inline')&&state().reason==='dirty'&&location.hash==='#/admin/dish/soup');
});
