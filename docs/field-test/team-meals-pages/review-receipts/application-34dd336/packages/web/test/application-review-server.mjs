...
const originalHtml=(await readFile(join(web,entry==='application'?'index.html':'test/pwa-browser.html'),'utf8')).replaceAll('../src/','./src/');
const html=originalHtml.replace('<script type="module" src="/src/main.ts"></script>','<script type="module" src="/test/application-review.ts"></script><script type="module" src="/src/main.ts"></script>');
const snapshots={};
for(const version of (process.env.C2B_VARIANTS??'a,b').split(',')) {
 const root=join(directory,version);await mkdir(root);
 await cp(join(web,'src'),join(root,'src'),{recursive:true});
 await mkdir(join(root,'test'));await cp(join(web,'test/application-review.ts'),join(root,'test/application-review.ts'));
 ...
 const fixture=publishedFixture(version==='empty'?'no-plans':version==='legacy'?'normal':`image-${version}`);
 if(version==='legacy')runBuild({root:fixture.root,commit:fixture.revision,target:'legacy-numeric',at:'September 11, 2026 00:00:00 GMT',write:true,outDir:join(root,'public/data')});else fixture.publish(join(root,'public/data'));
