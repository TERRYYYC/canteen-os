...
const [directory,out,reviewedHeadSha]=process.argv.slice(2),root=resolve(directory),require=createRequire(new URL('./packages/web/package.json',import.meta.url)),vreq=createRequire(require.resolve('vite/package.json'));
assert(/^[0-9a-f]{40}$/.test(reviewedHeadSha??''),'Explicit reviewed SHA is required');
