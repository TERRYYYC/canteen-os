...
- Actual in-app screenshots:plan-{zh,en,uk}-{393,1440}.png, each393×852or1440×900. Capture URL hides fixture controls but keeps explicit mock status. The initially named plan-flow-mobile-uk.png was captured after a development reload and actually shows the initial zh state. It is preserved as plan-initial-zh-mobile-harness.png; it is not proof of uk or recovery. The CUA interaction output observed PASS, and separate correctly timed flow evidence is added after dispatch caught this mismatch. Dark mode inherited system; light will be sampled during full-page acceptance.
