...
>
> **引用口径**：ADR-0007 已于 2026-09-08 **合并**（Status: Accepted）；`docs/specs/v03-worker-contract.md`（下文简称「worker 契约」）随本轮一并合并。worker 契约与本文冲突时**以 worker 契约为准**，本文跟进修订。正文里凡写「PR #61，待合并」「PR #64，待合并」的地方，一律改读作已合并；章节号未变，引用仍然有效。
>
> **决议追加（2026-09-08，Owner 当面拍板）——§9 里那三条阻塞级硬伤全部已解，正文尚未逐处回改，冲突时以本块为准：**
>
> - **硬伤 1 / D-09 已解**：worker 新增 `POST /dish/:id`，`status` 由请求体决定，**后台可直接建 `active`**；`POST /dish/:id/draft` 保持无条件写 `draft`。#24「手动加菜 → 入库 = active」按原样实现，**「入库」按钮不置灰**，D-09 里那段置灰 + 说明的设计作废。代价 Owner 已知悉并接受：少一道二次确认，schema 校验成为唯一闸门。
> - **硬伤 2 / D-01 已解**：令牌链接确定为 `…/#/admin/t/<token>`，ADR §4 已同步改掉初稿的 `…/admin#t=<token>`。本文 D-01 的方案被采纳，不再是「待 owner 裁」。
> - **硬伤 3 / D-05 已解**：worker 新增只读端点 `GET /catalog`（菜名 / 食材名 / 技法 / 供应商全库索引）与 `GET /changes`（未发布改动列表）。五屏的选择器与查重、#25 的发布记录都由它们供数，**不再需要前端自己拉全量 JSON**，首屏 60 KB gzip 预算的顾虑随之消失。已回灌 ADR-0007 §5 与 worker 契约。
> - **§9 图片压缩**：按 ADR 走 —— 浏览器 canvas 压到最长边 ≤ 1280 且 ≤ 200 KB，worker 只校验尺寸与 magic bytes。涉及 #23 拍照上传的那一屏按此实现。
> - **`workflow_dispatch` 已上线**（PR #67，`ee00a8a`）：#25 发布屏的四步进度有真实数据源了。前端传 `request_id`，worker 按 `run-name` 认领 runId（worker 契约 D-17）。
>
> §9 其余 12 条较轻的矛盾**仍然有效、仍需逐条处理**，不在本次决议范围内。
>
