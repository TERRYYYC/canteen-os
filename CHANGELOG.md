...
## [0.3.0-alpha.1] — 2026-09-13 · 团队用餐预发布

**能用了什么**

- 团队每日用餐计划与粘贴导入、备料、菜单和人工确认采购，采用中/英/乌三语界面；菜品、食材与实际配方可编辑，保留不完整资料的明确提示。
- 真实 HTTP 客户端与 Worker 接口已接线；编辑会话、并发冲突与未知写入结果有保护，发布资料和图片绑定同一提交版本。
- 页面可读性与主导航更新，二维码打印隐藏应用控件；抽屉底部新增由 Web 包版本派生的应用版本，与资料更新时间分别显示。
- CI 覆盖跨包契约、固定黄金夹具与同版发布产物；图片工具供应固定版本。发布配置先校验 Worker 地址，再交给前端构建；未配置时保持明确只读。
