...
| 鸡蛋 | 720 pcs | —（pcs 不套 yield） | ×1.1 = 792 pcs | 0 | ÷180 枚 = 4.4 | ceil→5 | 5 箱 ×180 = **900 枚** | ¥750.00 |
...
按供应商分单：**绿源农产品配送**（番茄+鸡蛋+小葱）¥1339.50；**宏达粮油调味批发**（食盐+食用油）¥186.00；
合计 **¥1525.50 / 480 份 ≈ ¥3.18/份**。完整推导规则见 [docs/modules/procurement.md](../../docs/modules/procurement.md) §2。

> **2026-09-06 修正（ADR-0006 §3 更正备注）**：margin（防少买系数）对**所有**食材生效，含 pcs——
> 鸡蛋行由「720 pcs 直通 → 4 箱 ¥600.00」修正为「×1.1 = 792 → 5 箱 ¥750.00」，合计由 ¥1375.50 改为
> ¥1525.50。只有 yield 不作用于 pcs。基准表每行均已逐格手算重核（黄金测试锁的是写下来的数字，
> 不是正确的数字——每行须手算）。
